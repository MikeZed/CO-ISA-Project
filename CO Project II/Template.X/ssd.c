...
void __ISR(_TIMER_1_VECTOR, ipl7) Timer1ISR(void) 
{
    ...
    if(baseCnt++ == 499)
   {
    ...
    /*המערך בודק לחיצץ כפתור בכל פסיקה
     מאפס את מספר השניות
     ומאתחל את ספירת הפסיקות לארבע מאות תשעים ותשע 
     כך שניכנס מיד לאיף שבודק את כמות הפסיקות
     ונדפיס שורת אפסים בהצגת שבעת הקווים*/
   if (btn_info[2])
    {
        wCnt=0;
        baseCnt= 499;
