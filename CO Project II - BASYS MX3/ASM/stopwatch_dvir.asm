...
hndl_int: 							# handles interrupts 
		...
		sub $sp, $sp, $t0,  0				# make room for 2 registers in stack 		
...
main: 
		...
		#play $zero, $zero, $zero, 0 		
		limm $sp, $zero, $zero, 300			# $sp = 300 
		
		limm $t0, $zero, $zero, 1 			# $t0 = 1 for IO Registers[0]
		
		limm $t1, $zero, $zero, 2			# $t1 = 2 for IO Registers[2] - hndl_int ?????? ??
		
		limm $t2, $zero, $zero, 226 			# $t2 = 200 for IO Registers[3]
...
cnt_time:	# updates time counters 
		...
		add $s1, $s1, $fp, 0				# $s1 += $fp
		limm $fp, $zero, $zero, 0			# $fp = 0 

		limm $t2, $zero, $zero, 59 			# $t2 = 59 
		branch ble, $s1, $t2, no_add_min 		# $s1 <= 59 -> a minute has not yet passed 
	
		limm $t3, $zero, $zero, 1 			# $t3 = 1 
		add $s2, $s2, $t3, 0				# $s2 += 1 
		add $t3, $t2, $t3, 0				# $t3 = 60 
		sub $s1, $s1, $t3, 0				# $s1 -= 60  
		
		branch blt $s2, $t3, no_add_min 		# $s2 < 60 -> no need to zero $s2 
		limm $s2, $zero, $zero, 0 			# $s2 = 0
...
prep_for_disp: 							# returns corrected number for display
		...
		sub $sp , $sp, $t0,  0				# make room for 3 registers in stack 	
